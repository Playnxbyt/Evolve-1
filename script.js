  ...
  list.innerHTML=CATS.map(cat=>{
    ...
    const goalsHTML=catGoals.length===0
      ...
      :catGoals.map((g,i)=>{
        ...
        return `<div class="goal-row" data-cat="${cat.id}" data-gid="${g.id}" data-carousel-index="${i}" onclick="toggleGoal('${cat.id}','${g.id}')" style="background:${isDone?cat.color+"20":"rgba(255,255,255,0.04)"};border-color:${isDone?cat.color+"60":"rgba(255,255,255,0.07)"}">
    ...
    return `<div class="cat-card${allDone?' all-done':''}" style="--cat-glow:${cat.color}66;background:${cat.bg};border:1px solid ${cat.color}28;box-shadow:${isExp?`0 0 28px ${cat.color}35`:"none"}">
      ...
      ${isExp?`<div class="goals-list"><div class="goals-list-inner">${goalsHTML}</div></div>`:""}
    </div>`;
  }).join("");
...
function toggleTaskCat(catId) {
  ...
  const statsEl = document.getElementById('task-month-stats');
  ...
  if (isOpen) {
    // ── Close ────────────────────────────────────
    panel.classList.remove('overlay-open');
    card.classList.remove('overlay-open');
    if (chevron) {
      chevron.style.transition = 'transform 0.4s cubic-bezier(0.34,1.2,0.64,1)';
    ...
    // Remove dim overlay class after transition
    setTimeout(() => {
      if (statsEl) statsEl.classList.remove('has-overlay-open');
    }, 420);

  } else {
    // ── Open ─────────────────────────────────────
    // Close any other open panels first
    document.querySelectorAll('.task-cat-goals.overlay-open').forEach(p => {
      const otherCard = p.closest('.task-month-cat-card');
      const otherChev = otherCard ? otherCard.querySelector('.task-month-chevron') : null;
      p.classList.remove('overlay-open');
      if (otherCard) otherCard.classList.remove('overlay-open');
      if (otherChev) otherChev.style.transform = 'rotate(0deg)';
      p.querySelectorAll('.task-goal-row').forEach(r => r.classList.remove('row-visible'));
    });

    // Apply dim backdrop to container
    if (statsEl) statsEl.classList.add('has-overlay-open');

    // Inherit the card's background for the overlay panel
    const cardBg = card.style.background || 'rgba(20,16,35,0.98)';
    panel.style.background = cardBg || '#1a1228';

    // Open panel with overlay animation
    requestAnimationFrame(() => {
      panel.classList.add('overlay-open');
      card.classList.add('overlay-open');

      if (chevron) {
        chevron.style.transition = 'transform 0.45s cubic-bezier(0.34,1.4,0.64,1)';
        chevron.style.transform  = 'rotate(90deg)';
      }

      // Stagger goal rows sliding in from left
          ...
          return `<div class="task-month-cat-card" style="border-color:${cat.color}22;background:${cat.bg}">
            <div class="task-month-cat-header" onclick="toggleTaskCat('${cat.id}')">
              ...
              <div style="display:flex;align-items:center;gap:8px">
                ...
                <span class="task-month-chevron" style="color:rgba(255,255,255,0.3);font-size:13px;display:inline-block">›</span>
              </div>
            </div>
            ...
            <div class="task-cat-goals" id="${expandId}">${goalRows}</div>
          </div>`;
        }).join("");
    }
  }
...
function toggleExpand(catId) {
  ...
  // Card scale pulse on tap
  const cards = document.querySelectorAll('.cat-card');
  cards.forEach(card => {
    const header = card.querySelector('.cat-header');
    if (header && header.getAttribute('onclick') && header.getAttribute('onclick').includes(`'${catId}'`)) {
      card.classList.add('expanding');
      setTimeout(() => card.classList.remove('expanding'), 500);
    }
  });

  renderHome();

  // After render, stagger in goal rows with carousel effect
  if (!wasExpanded) {
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        const rows = document.querySelectorAll(`.goal-row[data-cat="${catId}"]`);
        rows.forEach((row, i) => {
          setTimeout(() => {
            row.classList.add('carousel-visible');
          }, i * 75 + 30);
        });
      });
    });
