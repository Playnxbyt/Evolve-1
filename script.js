...
function renderHome() {
  ...
  list.innerHTML=CATS.map(cat=>{
    ...
    return `<div class="cat-card${allDone?' all-done':''}" data-catid="${cat.id}" style="--cat-glow:${cat.color}66;background:${cat.bg};border:1px solid ${cat.color}28;box-shadow:${isExp?`0 0 28px ${cat.color}35`:"none"}">
      ...
      ${isExp
        ? `<div class="goals-list goals-list-animating">${goalsHTML}</div>`
        : `<div class="goals-list" style="display:none"></div>`}
    </div>`;
  }).join("");
  ...
  // Animate any newly-opened goals-list panels
  requestAnimationFrame(() => {
    document.querySelectorAll(".goals-list-animating").forEach(panel => {
      panel.classList.remove("goals-list-animating");
      const fullH = panel.scrollHeight;
      panel.style.cssText = "height:0;opacity:0;overflow:hidden;padding:0 18px;box-sizing:border-box;display:flex;flex-direction:column;gap:8px;border-top:1px solid rgba(255,255,255,0.06)";
      panel.offsetHeight; // reflow
      panel.style.transition = "height 0.42s cubic-bezier(0.4,0,0.2,1), opacity 0.32s ease";
      panel.style.height = (fullH + 28) + "px";
        ...
        CATS.map(cat=>{
          ...
          const goalRows=catGoals.map(g=>{
            ...
            for(let dd=1;dd<=daysElapsed;dd++){
              ...
              if(isToday2){
                // Use the actual per-goal done state for today
                if(state.done[`${cat.id}-${g.id}`]) doneCount++;
              } else {
                // Use per-goal historical data stored in monthData under a goal-specific key
                const goalKey=`${cat.id}-${g.id}`;
                const dd_data=state.monthData[k2];
                if(dd_data&&dd_data[goalKey]) doneCount++;
              }
            }
...
function toggleExpand(catId) {
  ...
  // Find the card and animate its goals panel without a full re-render
  ...
  if (card) {
    ...
    if (wasExpanded) {
      ...
      if (panel) {
        ...
        panel.style.transition = "height 0.38s cubic-bezier(0.4,0,0.2,1), opacity 0.28s ease";
        panel.style.height = "0";
        panel.style.opacity = "0";
        panel.addEventListener("transitionend", () => {
          panel.style.display = "none";
      ...
      // The goals list doesn't exist yet if card was collapsed — do a targeted re-render
      renderHome();
      return;
    }
  } else {
