...
function renderMonthlyLineGraph() {
  ...
  if(footer){
    ...
    footer.innerHTML=`
      ...
      <div class="monthly-stat"><span class="monthly-stat-val" style="color:#FBBF24">${daysToShow}</span><span class="monthly-stat-lbl">Days In</span></div>
      <div class="monthly-stat"><span class="monthly-stat-val" style="color:#34D399">${daysInMonth}</span><span class="monthly-stat-lbl">Days Total</span></div>
    `;
  }
  ...
  if(pts.length<2){
    ...
    ctx.fillText("Check off goals to see your monthly trend",W/2,H/2);
    return;
  }
  ...
  ctx.beginPath();
  ctx.moveTo(getX(pts[0].d),getY(pts[0].pct));
  for(let i=1;i<pts.length;i++){
    const prev=pts[i-1],curr=pts[i];
    const cpx=(getX(prev.d)+getX(curr.d))/2;
    ctx.bezierCurveTo(cpx,getY(prev.pct),cpx,getY(curr.pct),getX(curr.d),getY(curr.pct));
  }
  ...
  pts.forEach((p,i)=>{
    ...
    ctx.beginPath();
    ctx.arc(cx,cy,isLast?5:2.5,0,Math.PI*2);
    ctx.fillStyle=isLast?"#E879F9":"rgba(167,139,250,0.7)";
    if(isLast){ctx.shadowColor="#E879F9";ctx.shadowBlur=10;}
    ctx.fill();
    ctx.shadowBlur=0;
    if(isLast){
      // Label last point
      ...
      ctx.textAlign="center";
      const labelY=cy<PAD.top+16?cy+16:cy-10;
      ctx.fillText(p.pct+"%",cx,labelY);
    }
  });

  // X-axis day labels (show every 5 days)
  ctx.fillStyle="rgba(255,255,255,0.25)";
  ctx.font="8px 'Space Mono',monospace";
  ctx.textAlign="center";
  for(let d=1;d<=daysToShow;d+=5){
    ctx.fillText(d,getX(d),H-4);
  }
  // Always show last day
  ctx.fillText(daysToShow,getX(daysToShow),H-4);
}
...
function renderCalendar() {
  ...
  if(taskStats){
    // Build per-goal stats across all categories
    const allGoals=[];
    CATS.forEach(cat=>{
      state.goals[cat.id].forEach(g=>{
        // Count completions this month for this goal
        let doneCount=0;
        for(let dd=1;dd<=daysElapsed;dd++){
          ...
          if(isToday2){
            ...
            // Historical: we only have category-level counts, not per-goal
            // Approximate: if category had completions that day, count proportionally
            const dd_data=state.monthData[k2];
            if(dd_data&&dd_data[cat.id]>0){
              const catTotal=state.goals[cat.id].length;
              if(catTotal>0) doneCount+=dd_data[cat.id]/catTotal;
            }
          }
        }
        const pct=daysElapsed>0?Math.round((doneCount/daysElapsed)*100):0;
        allGoals.push({cat,goal:g,doneCount:Math.round(doneCount),daysElapsed,pct});
      });
    });
    ...
    if(allGoals.length===0){
      ...
      taskStats.innerHTML=`<div class="task-month-header">Goal Completion This Month</div>`+
        allGoals.map(({cat,goal,doneCount,daysElapsed,pct})=>`
        <div class="task-month-card" style="border-color:${cat.color}22;background:${cat.bg}">
          <div class="task-month-top">
            <span class="task-month-emoji">${cat.emoji}</span>
            <span class="task-month-name">${goal.text}</span>
            <span class="task-month-pct" style="color:${cat.color}">${pct}%</span>
          </div>
          <div class="task-month-fraction" style="color:rgba(255,255,255,0.35)">${doneCount} / ${daysElapsed} days</div>
          <div class="task-month-bar-track">
            <div class="task-month-bar-fill" style="width:${pct}%;background:linear-gradient(90deg,${cat.color}88,${cat.color})"></div>
          </div>
        </div>
      `).join("");
    }
