...
function renderCalendar() {
  ...
  // For the fraction display, always show total days in the month (30/31 etc.)
  // For counting completed days, only count up to today for the current month
  const daysElapsed=isCurrentMonth2?today2.getDate():daysInMonth2;
  const daysDisplay=daysInMonth2;
          ...
          const goalRows=catGoals.map(g=>{
            ...
            return `<div class="task-goal-row">
              ...
              <div class="task-goal-right">
                <span class="task-goal-frac" style="color:rgba(255,255,255,0.35)">${gDone}/${daysDisplay}d</span>
...
function toggleGoal(catId,goalId) {
  ...
  // Save BEFORE rendering so all reads see correct state
  saveData();

  if(state.done[k]) playSound('check');
  ...
  // Re-render (preserves expanded state via state.expanded)
  renderHome();
  renderCalendar();

  // Highlight the toggled row after DOM is rebuilt
  if(state.done[k]){
