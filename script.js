...
function switchView(v) {
  ...
  if(v==="stats") {
    ...
    renderBadges(); renderInsights(); renderXP(); renderMonthlyGraph();
  }
}
...
function renderMonthlyGraph() {
  ...
  let series = [];
  if(tab==="combined"){
    const data = Array.from({length:daysInMonth},(_,i)=>getDayScore(y,m,i+1));
    series = [{data, color:"#A78BFA", label:"All"}];
  } else {
    const cat = CATS.find(c=>c.id===tab);
    if(!cat) return;
    const data = Array.from({length:daysInMonth},(_,i)=>{
      const k=`${y}-${m}-${i+1}`;
      return (state.monthData[k]||{})[tab]||0;
    });
    series = [{data, color:cat.color, label:cat.label}];
  }
  ...
  for(let i=0;i<=gridLines;i++){
    const yy = padT + chartH - (i/gridLines)*chartH;
    ctx.beginPath(); ctx.moveTo(padL,yy); ctx.lineTo(W-padR,yy); ctx.stroke();
  }
  ctx.setLineDash([]);

  // Draw bars (combined) or line (category)
  const barW = Math.max(3, (chartW / daysInMonth) - 2);
  ...
  series.forEach(s=>{
    if(tab==="combined"){
      ...
      s.data.forEach((val,i)=>{
        if(val===0) return;
        const x = padL + (i/daysInMonth)*chartW + (chartW/daysInMonth - barW)/2;
        const barH = (val/maxVal)*chartH;
        const yy = padT + chartH - barH;
        ...
        ctx.beginPath();
        ctx.moveTo(x+r,yy);ctx.lineTo(x+barW-r,yy);
        ctx.quadraticCurveTo(x+barW,yy,x+barW,yy+r);
        ctx.lineTo(x+barW,yy+barH);ctx.lineTo(x,yy+barH);ctx.lineTo(x,yy+r);
        ctx.quadraticCurveTo(x,yy,x+r,yy);
        ctx.closePath();ctx.fill();
        // Glow on today
        const isToday=i+1===today.getDate()&&m===today.getMonth()&&y===today.getFullYear();
        if(isToday){
          ctx.strokeStyle="rgba(232,121,249,0.8)";ctx.lineWidth=1.5;ctx.stroke();
        }
      });
    } else {
      // Smooth line chart
      ...
      const pts = s.data.map((val,i)=>({
        x: padL + (i+0.5)*(chartW/daysInMonth),
      ...
      fillGrad.addColorStop(0,color+"50");
      ...
      pts.forEach((p,i)=>{
        if(s.data[i]>0){
          ...
          ctx.strokeStyle="rgba(8,6,15,0.8)";ctx.lineWidth=1.5;ctx.stroke();
        }
      });
    }
  });
  ...
  if(labelsEl){
    const step = daysInMonth<=20?2:daysInMonth<=25?3:5;
    labelsEl.innerHTML = Array.from({length:daysInMonth},(_,i)=>{
      const d=i+1;
      const show=d===1||d%step===0||d===daysInMonth;
      const isWeekend=[0,6].includes(new Date(y,m,d).getDay());
      return `<span class="monthly-chart-label${isWeekend?" weekend":""}" style="opacity:${show?1:0}">${d}</span>`;
    }).join("");
  }
  ...
  if(summaryEl){
    const daysWithData = Array.from({length:daysInMonth},(_,i)=>getDayScore(y,m,i+1)).filter(v=>v>0).length;
    ...
    const colors = tab==="combined"?"#A78BFA":(CATS.find(c=>c.id===tab)||{color:"#A78BFA"}).color;
    summaryEl.innerHTML=`
      <div class="monthly-summary-cell">
        <div class="monthly-summary-val" style="color:${colors}">${total}</div>
        <div class="monthly-summary-lbl">Total</div>
      </div>
      <div class="monthly-summary-cell">
        <div class="monthly-summary-val" style="color:${colors}">${best}</div>
        <div class="monthly-summary-lbl">Best Day</div>
      </div>
      <div class="monthly-summary-cell">
        <div class="monthly-summary-val" style="color:${colors}">${avg}</div>
        <div class="monthly-summary-lbl">Daily Avg</div>
      </div>
      <div class="monthly-summary-cell">
        <div class="monthly-summary-val" style="color:${colors}">${pct}%</div>
