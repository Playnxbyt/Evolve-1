...
function renderHome() {
  ...
  list.innerHTML=CATS.map(cat=>{
    ...
    return `<div class="cat-card${allDone?' all-done':''}${isExp?' expanded':''}" data-cat-id="${cat.id}" style="--cat-glow:${cat.color}66;background:${cat.bg};border:1px solid ${cat.color}28;box-shadow:${isExp?`0 0 28px ${cat.color}35`:"none"}">
      <div class="cat-header" onclick="toggleExpand('${cat.id}')">
        ...
        <span class="cat-chevron" style="${isExp?'transform:rotate(90deg);color:rgba(255,255,255,0.6)':''}">›</span>
      ...
      <div class="goals-list-outer${isExp?' open':''}">
...
function toggleTaskCat(catId) {
  ...
  if (isOpen) {
    ...
    card && card.classList.remove('expanded-card');
    // rows fade out first via CSS transition reversal
    panel.querySelectorAll('.task-goal-row').forEach(row => {
      row.style.transitionDelay = '0s';
    });
  } else {
    // ── Expand ──────────────────────────────────
    // Stagger rows on the way in
    panel.querySelectorAll('.task-goal-row').forEach((row, i) => {
      row.style.transitionDelay = (0.08 + i * 0.06) + 's';
    });
    panel.classList.add('expanded');
    card && card.classList.add('expanded-card');
  }
}
...
function toggleExpand(catId) {
  ...
  // Smooth-animate: find the outer wrapper in current DOM
  const card = document.querySelector(`.cat-card[data-cat-id="${catId}"]`);
  if (card) {
    const outer = card.querySelector('.goals-list-outer');
    ...
    if (outer) {
      if (wasExpanded) {
        ...
        // Stagger goal rows inside
        outer.querySelectorAll('.goal-row').forEach((row, i) => {
...
// ═══════════════════════════════════════════════
//  CAROUSEL — home category cards
// ═══════════════════════════════════════════════
function initCarousel() {
  const track = document.getElementById('cats-list');
...
// Re-init carousel whenever home re-renders
