...
function getCatMonthCount(catId, y, m){
  if(y===undefined){ y=today.getFullYear(); m=today.getMonth(); }
  const prefix=`${y}-${m}-`;
  return Object.entries(state.monthData)
    .filter(([k])=>k.startsWith(prefix))
    .reduce((s,[,d])=>s+(d[catId]||0),0);
}
...
function renderInsights() {
  const y=today.getFullYear(), m=today.getMonth();
  const sorted=[...CATS].sort((a,b)=>getCatMonthCount(b.id,y,m)-getCatMonthCount(a.id,y,m));
...
function renderCatStats() {
  ...
  cards.innerHTML=`<div class="cat-stat-section-label">Category Breakdown</div>`+CATS.map(cat=>{
    const catGoals=state.goals[cat.id];
    const mc=getCatMonthCount(cat.id,y,m);
...
function renderCalendar() {
  ...
  const maxCount=Math.max(...CATS.map(c=>getCatMonthCount(c.id,y,m)),1);
  document.getElementById("cat-bars").innerHTML=CATS.map(cat=>{
    const cnt=getCatMonthCount(cat.id,y,m);
